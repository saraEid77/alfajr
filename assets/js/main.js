jQuery(function ($) {
    'use strict';
		...
		if( $ (this).hasClass('inOut')  ){
			$('.side-options').stop().animate({right:'0px'},500 );
		} else{
			$('.side-options').stop().animate({right:'-200px'},500 );
		} 
		...
		$('.o-icon2').click (function(event){
			event.preventDefault();
			if( $ (this).hasClass('inOut')  ){
				$('.side-options2').stop().animate({right:'0px'},500 );
			} else{
				$('.side-options2').stop().animate({right:'-300px'},500 );
			} 
			$(this).toggleClass('inOut');
			return false;
       ...
       // responsive tabs
		 ...
		 $('#tab_selector').on('change', function (e) {
			$('.nav-tabs button').eq($(this).val()).tab('show');
		});
